I'm ready to start porting this project over for my work, but I have run into a lot of trouble getting the SharePoint server and MS Graph instance set up.
My boss asked me to write an email to send to the head honcho to get him to motivate the server monkeys to get to work on it.
Part of the problem is that the server monkeys don't know exactly what to do (this isnnt something that they do every day), and part of the issue is that they don't really care.
We don't need to worry about motivating them. The BigDog will do that.
We just need to write a multi-part email (tl;dr, more detail, technical task list) to send. Markdown and HTML don't do well in our email, so we need to keep the formatting simple.
...
## src\GraphLib.PdfConsoleNet48\GraphLibRunner.cs
